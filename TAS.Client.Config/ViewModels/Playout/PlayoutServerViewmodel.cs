...
namespace TAS.Client.Config.ViewModels.Playout
{
    public class PlayoutServerViewModel : OkCancelViewModelBase
    {
        ...
        private void Init()
        {
            ServerAddress = PlayoutServer.ServerAddress;
            OscPort = PlayoutServer.OscPort;
            MediaFolder = PlayoutServer.MediaFolder;
            IsMediaFolderRecursive = PlayoutServer.IsMediaFolderRecursive;
            AnimationFolder = PlayoutServer.AnimationFolder;
            ServerType = PlayoutServer.ServerType;
            MovieContainerFormat = PlayoutServer.MovieContainerFormat;
            IsModified = false;
        }
        ...
        public Model.CasparServer PlayoutServer { get; }
        ...
        private void _playoutRecorders_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                PlayoutServer.Recorders.Add(((PlayoutRecorderViewModel)e.NewItems[0]).CasparRecorder);
            }
            if (e.Action == NotifyCollectionChangedAction.Remove)
            {
                PlayoutServer.Recorders.Remove(((PlayoutRecorderViewModel)e.OldItems[0]).CasparRecorder);
            }
        ...
        private void _playoutServerChannels_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                PlayoutServer.Channels.Add(((PlayoutServerChannelViewModel)e.NewItems[0]).CasparServerChannel);
            }
            if (e.Action == NotifyCollectionChangedAction.Remove)
            {
                PlayoutServer.Channels.Remove(((PlayoutServerChannelViewModel)e.OldItems[0]).CasparServerChannel);
            }
            _isAnyChildChanged = true;
        }
        ...
        public override bool Ok(object obj = null)
        {
            ...
            PlayoutServer.AnimationFolder = AnimationFolder;
            PlayoutServer.IsMediaFolderRecursive = IsMediaFolderRecursive;
            PlayoutServer.MediaFolder = MediaFolder;
            PlayoutServer.MovieContainerFormat = MovieContainerFormat;
            PlayoutServer.OscPort = OscPort;
            PlayoutServer.ServerAddress = ServerAddress;
            PlayoutServer.ServerType = ServerType;            
            return true;
        }
