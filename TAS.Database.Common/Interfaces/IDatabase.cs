...
namespace TAS.Database.Common.Interfaces
{
    public interface IDatabase
    {
        ...
        IList<T> FindArchivedStaleMedia<T>(IArchiveDirectoryServerSide dir) where T : IArchiveMedia, new();
        ...
        IList<T> LoadSecurityObject<T>() where T : ISecurityObject;
        IReadOnlyCollection<T> LoadArchiveDirectories<T>() where T : IArchiveDirectoryProperties, new();
        IReadOnlyCollection<T> LoadEngines<T>(ulong? instance = null) where T : IEnginePersistent;
        IReadOnlyCollection<T> LoadServers<T>() where T : IPlayoutServerProperties;
        ...
        IList<IAclRight> ReadEngineAclList<TEngineAcl>(IPersistent engine, IAuthenticationServicePersitency authenticationService) where TEngineAcl : IAclRight, IPersistent, new();
        IEvent ReadEvent(IEngine engine, ulong idRundownEvent);
        IList<IAclRight> ReadEventAclList<TEventAcl>(IEventPersistent aEvent, IAuthenticationServicePersitency authenticationService) where TEventAcl : IAclRight, IPersistent, new();
        IEvent ReadNext(IEngine engine, IEventPersistent aEvent);
        void ReadRootEvents(IEngine engine);
        IList<IEvent> ReadSubEvents(IEngine engine, IEventPersistent eventOwner);
        ulong SaveMediaSegment(IMediaSegment mediaSegment);
        IList<T> ArchiveMediaSearch<T>(IArchiveDirectoryServerSide dir, TMediaCategory? mediaCategory, string search) where T : IArchiveMedia, new();
        int CheckDatabase(IEngine engine, bool recoverLostEvents);
        IList<IEvent> SearchPlaying(IEngine engine);
